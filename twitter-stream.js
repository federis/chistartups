...
// Keep array of twitter ids
var twitter_ids = []

_.each(watchStartups, function(twitter_account) {
  twitter_ids.push(twitter_account.twitter_id)
})
...
t.stream('statuses/filter', { follow: twitter_ids }, function(stream) {
  ...
  stream.on('data', function(tweet) {
    ...
    if (startup.length > 0) {
      ...
      tweetsList.push(JSON.stringify(tweet_info))
